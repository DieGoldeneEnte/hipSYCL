...
int main() {
  ...
  //kernels
  auto node_ptr1    = std::make_shared<dag_node>(no_hint, no_requirements,
                                              std::unique_ptr<operation>(&kernel_op));
  auto node_memcpy1 = std::make_shared<dag_node>(
      no_hint, std::vector{node_ptr1}, std::unique_ptr<operation>(&test_memcpy));
  auto node_ptr2 =
      std::make_shared<dag_node>(no_hint, std::vector{node_ptr1, node_memcpy1},
                                 std::unique_ptr<operation>(&kernel_op));
  auto node_ptr3 = std::make_shared<dag_node>(no_hint, std::vector{node_ptr2, node_ptr1},
                                              std::unique_ptr<operation>(&kernel_op));
  //Memcpy
